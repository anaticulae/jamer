...
def test_pdf_pagenumber():
    ...
    pages = jamer.pdf.pagenumber(hoverpower.MASTER072_PDF)
    assert pages == 72


def test_pdf_remove():
    """Remove first 10 pages."""
    removed = jamer.pdf.remove(hoverpower.MASTER072_PDF, tuple(range(10)))
...
def test_pdf_select():
    ...
    selected = jamer.pdf.select(hoverpower.MASTER072_PDF, (69, 70, 71))
...
def test_pdf_switch():
    ...
    switched = jamer.pdf.switch(hoverpower.MASTER072_PDF, pages=flip)
    pages = jamer.pdf.pagenumber(switched)
    assert pages == 72

    before = jamer.pdf.hashcontent(hoverpower.MASTER072_PDF)
    after = jamer.pdf.hashcontent(switched)
    assert after != before


def test_pdf_switch_empty():
    switched = jamer.pdf.switch(hoverpower.MASTER072_PDF)
    before = jamer.pdf.hashcontent(hoverpower.MASTER072_PDF)
    after = jamer.pdf.hashcontent(switched)
